...
#include "../interface/RooEmpXHistShape.h"
  ...
  _refRangeName(0)
{
  for (Int_t i = 0; i < coefList.getSize(); ++i) {
    const RooAbsArg& coef = coefList[i];
...
Double_t RooEmpXHistShape::bernstein(Double_t x_) const {
  ...
  auto coefVal = [this](Int_t i) -> Double_t {
    return static_cast<const RooAbsReal&>(_coefList[i]).getVal();
  };
  ...
  if(degree == 0) {
    ...
    Double_t a0 = coefVal(0); // c0
    Double_t a1 = coefVal(1) - a0; // c1 - c0
    return a1 * x_norm + a0;
 
  } else if(degree == 2) {
 
    Double_t a0 = coefVal(0); // c0
    Double_t a1 = 2 * (coefVal(1) - a0); // 2 * (c1 - c0)
    Double_t a2 = coefVal(2) - a1 - a0; // c0 - 2 * c1 + c2
    ...
    Double_t result = coefVal(0) * s;
    for(Int_t i = 1; i < degree; i++) {
      result = (result + t * TMath::Binomial(degree, i) * coefVal(i)) * s;
      t *= x_norm;
    }
    result += t * coefVal(degree);
...
Double_t RooEmpXHistShape::polynomial(Double_t x_) const{
  ...
  _wksp.reserve(sz);
  {
    ...
    for (unsigned i = 0; i < sz; ++i) {
      _wksp.push_back(static_cast<const RooAbsReal&>(_coefList[i]).getVal(nset));
    }
  }
  const Double_t x = x_;
